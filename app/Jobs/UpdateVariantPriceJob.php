...
class UpdateVariantPriceJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function __construct(
        ...
        $this->idempotencyKey = hash('sha256', implode(':', [
            ...
            $variantId,
            $price,
            $discountPrice !== null ? sprintf('discount-%d', $discountPrice) : 'discount-null',
            $hasRawPrice !== null ? sprintf('raw-%s', $hasRawPrice ? 'true' : 'false') : 'raw-null',
        ]));
    }
    ...
    public function handle(
        ...
        try {
            ...
            if ($exception->isClientError() && $exception->statusCode() !== 409) {
                ...
                $recordEvent->execute($this->runId, 'VALIDATION_FAILED', array_filter([
                    'variant_id' => $this->variantId,
                    'price' => $this->price,
                    'status' => $exception->statusCode(),
    ...
    private function persistFailure(string $message, array $extraPayload = []): void
    {
        ...
        if ($this->discountPrice !== null) {
            $payload['discount_price'] = $this->discountPrice;
        }

        if ($this->hasRawPrice !== null) {
